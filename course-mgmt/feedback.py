...
def main():
    ...
    parser.add_argument(
        "--dry-run", "-n", action="store_true", help="limit to assignment ID"
    )
    ...
    parser.add_argument(
        ...
        action="append",
        help=(
            "limit to these usernames (comma separated list, "
            "or can be given multiple times)"
        ),
    )
    parser.add_argument(
        "--user-exclude", action="append", help=("Exclude these usernames.")
    )
    parser.add_argument(
        "--feedback_dir_format",
        type=int,
    ...
    for user_source_path in user_paths:
        ...
        if args.feedback_dir_format == 1:
            ...
            cmd = [
                ...
                "-r",
                "--update",
                "-i" + ("n" if args.dry_run else ""),
                "-og",
                "--chown=%s:%s" % (uid, USER_GID),
                "--perms",
                "--chmod=u+rwX,g=,g-s,o=",
                *assignment_limit,
                str(user_source) + "/",
                str(
                    Path(USERDIR.format(digits="%02d" % (uid % 100), username=username))
                    / course_slug
                    / "feedback"
                )
                + "/",
            ...
            for assignment_dir in glob.glob(str(user_source / "*")):
                ...
                timestamp = (
                    open(str(assignment_dir / "timestamp.txt"))
                    .read()
                    .strip()
                    .replace("/", "")
                )
            ...
            raise RuntimeError(
                "unknown feedback dir format: %s" % args.feedback_dir_format
            )
