...
def get_stats(get):
    ...
    # List all users
    r = get("users")
    STATUS["users_active"] = sum(
        [1 if user["server"] or user["pending"] else 0 for user in r]
    )
    ...
    if LAST_SUCCESSFUL_SPAWN_TIME:
        ...
        STATUS["spawn_test_successful"] = (
            now - LAST_SUCCESSFUL_SPAWN_TIME
        ) < INTERVAL_SPAWN_LIMIT
    else:
...
def make_prom_line(key, val, labels={}):
    ...
    for label_key, label in labels.items():
        if isinstance(label, str) and label.startswith("le_"):
            ...
            label_key = "le"
            label = label[3:]
        label_list.append('{}="{}"'.format(label_key, label))
                    ...
                    for key, val in STATUS.items():
                        if isinstance(val, dict):
                            for dict_key, dict_val in val.items():
                                output += make_prom_line(
                                    key, dict_val, labels={"sub_type": dict_key}
                                )
                        else:
